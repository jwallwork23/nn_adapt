// Domain and turbine specification
L = 1200.0;
W = 500.0;
D = 18.0;
dx_outer = 40.0;
dx_inner = 8.0;
xt0=500.0;  // x-location of turbine 1
yt0=286.0;  // y-location of turbine 1
xt1=600.0;  // x-location of turbine 2
yt1=250.0;  // y-location of turbine 2
xt2=700.0;  // x-location of turbine 3
yt2=214.0;  // y-location of turbine 3
xt3=500.0;  // x-location of turbine 4
yt3=214.0;  // y-location of turbine 4
xt4=700.0;  // x-location of turbine 5
yt4=286.0;  // y-location of turbine 5

// Domain and turbine footprints
Point(1) = {0, 0, 0, dx_outer};
Point(2) = {L, 0, 0, dx_outer};
Point(3) = {L, W, 0, dx_outer};
Point(4) = {0, W, 0, dx_outer};
Line(1) = {1, 2};
Line(2) = {2, 3};
Line(3) = {3, 4};
Line(4) = {4, 1};
Physical Line(1) = {4};   // Left boundary
Physical Line(2) = {2};   // Right boundary
Physical Line(3) = {1,3}; // Sides
Line Loop(1) = {1, 2, 3, 4};  // outside loop
Point(5) = {xt0-D/2, yt0-D/2, 0., dx_inner};
Point(6) = {xt0+D/2, yt0-D/2, 0., dx_inner};
Point(7) = {xt0+D/2, yt0+D/2, 0., dx_inner};
Point(8) = {xt0-D/2, yt0+D/2, 0., dx_inner};
Point(9) = {xt1-D/2, yt1-D/2, 0., dx_inner};
Point(10) = {xt1+D/2, yt1-D/2, 0., dx_inner};
Point(11) = {xt1+D/2, yt1+D/2, 0., dx_inner};
Point(12) = {xt1-D/2, yt1+D/2, 0., dx_inner};
Point(13) = {xt2-D/2, yt2-D/2, 0., dx_inner};
Point(14) = {xt2+D/2, yt2-D/2, 0., dx_inner};
Point(15) = {xt2+D/2, yt2+D/2, 0., dx_inner};
Point(16) = {xt2-D/2, yt2+D/2, 0., dx_inner};
Point(17) = {xt3-D/2, yt3-D/2, 0., dx_inner};
Point(18) = {xt3+D/2, yt3-D/2, 0., dx_inner};
Point(19) = {xt3+D/2, yt3+D/2, 0., dx_inner};
Point(20) = {xt3-D/2, yt3+D/2, 0., dx_inner};
Point(21) = {xt4-D/2, yt4-D/2, 0., dx_inner};
Point(22) = {xt4+D/2, yt4-D/2, 0., dx_inner};
Point(23) = {xt4+D/2, yt4+D/2, 0., dx_inner};
Point(24) = {xt4-D/2, yt4+D/2, 0., dx_inner};
Line(5) = {5, 6};
Line(6) = {6, 7};
Line(7) = {7, 8};
Line(8) = {8, 5};
Line(9) = {9, 10};
Line(10) = {10, 11};
Line(11) = {11, 12};
Line(12) = {12, 9};
Line(13) = {13, 14};
Line(14) = {14, 15};
Line(15) = {15, 16};
Line(16) = {16, 13};
Line(17) = {17, 18};
Line(18) = {18, 19};
Line(19) = {19, 20};
Line(20) = {20, 17};
Line(21) = {21, 22};
Line(22) = {22, 23};
Line(23) = {23, 24};
Line(24) = {24, 21};
Line Loop(2) = {5, 6, 7, 8};  // inside loop 1
Line Loop(3) = {9, 10, 11, 12};  // inside loop 2
Line Loop(4) = {13, 14, 15, 16};  // inside loop 3
Line Loop(5) = {17, 18, 19, 20};  // inside loop 4
Line Loop(6) = {21, 22, 23, 24};  // inside loop 5

// Surfaces
Plane Surface(1) = {1, 2, 3, 4, 5, 6};
Plane Surface(2) = {2};
Plane Surface(3) = {3};
Plane Surface(4) = {4};
Plane Surface(5) = {5};
Plane Surface(6) = {6};
Physical Surface(1) = {1};  // outside turbine
Physical Surface(2) = {2};  // inside turbine 1
Physical Surface(3) = {3};  // inside turbine 2
Physical Surface(4) = {4};  // inside turbine 3
Physical Surface(5) = {5};  // inside turbine 4
Physical Surface(6) = {6};  // inside turbine 5
