...
def go_metric_one2n(
    ...
    **kwargs,
    ...
    tt_steps = len(out["forward"])
    out["times"]["metric"] = -perf_counter()
    with PETSc.Log.Event("Metric construction"):
        if anisotropic:
            hessian_list = []
            for step in range(tt_steps):
                hessian_list.append(combine_metrics(*get_hessians(out["forward"][step]), average=average))
            hessian = time_integrate(hessian_list)
        else:
            hessian = None
