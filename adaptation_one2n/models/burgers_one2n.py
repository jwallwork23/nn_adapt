...
class Solver_one2n(nn_adapt.solving.Solver):
    ...
    def adjoint_iteration(self):
        """
        Get the adjoint solutions of Burgers equation
        """
    ...
    def test(self):
        ...
        fwd_sol = self._solutions
        F = self.form
        q_star = Function(self.V)
        adj_sol = []
        
        J = get_qoi(self.mesh)(fwd_sol[-1])
        dFdq = derivative(F, fwd_sol[-1], TrialFunction(self.V))
        dFdq_transpose = adjoint(dFdq)
        dJdq = derivative(J, fwd_sol[-1], TestFunction(self.V))
        solve(dFdq_transpose == dJdq, q_star)
        adj_sol.append(q_star)
        
        for i in range(2, self.tt_steps):
            remainder = product(dFdq_transpose, q_star)
            
            J = get_qoi(self.mesh)(fwd_sol[-i])
            dFdq = derivative(F, fwd_sol[-i], TrialFunction(self.V))
            dFdq_transpose = adjoint(dFdq)
            dJdq = derivative(J, fwd_sol[-i], TestFunction(self.V))
            dJdq -= remainder
            solve(dFdq_transpose == dJdq, q_star)
            adj_sol.append(q_star)
